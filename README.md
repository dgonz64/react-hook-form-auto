...
If you are not using your own skin, the one that comes with the library uses styles. The styles prop must be provided with each `<Autoform />` render. A way to avoid scaffolding is to put your `<Autoform />` in a module or use a HoC. Example for module:
