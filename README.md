...
## [Play with the demo](https://github.com/dgonz64/react-hook-form-auto-demo)
## [Documentation](#docs)
...
#### 2. Make your Autoform component

If you are not using your own skin, the one provided with the library uses styles. The styles prop must be provided with each `<Autoform />` render. A way to avoid scaffolding is to put your `<Autoform />` in a module or use a HoC. Example for module:
...
```javascript
    ...
    const styles = { ...panelStyles, ...inputStyles }
...
In this example I use sass and css-modules but you can provide the class names in any other way.

If sass suits you, you can copy-paste both demo styles: [inputs](https://github.com/dgonz64/react-hook-form-auto-demo/blob/master/src/components/inputs.sass) and [panels](https://github.com/dgonz64/react-hook-form-auto-demo/blob/master/src/components/Panel.sass). They are separated in the demo to reuse the panels. Sorry about it, working on it.

##### If you don't have sass

The classes inserted in the component are the ones specified in the `styles` object, like this:
...
For a complete list best take a look at former sass style files and adapt to your project layout.
...
It also allows you to build arrays from other schemas. You just toss an array with another schema from elsewhere as first element and `Autoform` will allow you to add and remove elements:

```javascript
    import { Schema } from 'react-hook-form-auto'
...
Read the [documentation](#docs) to find out what else you can do.
...
Actually I don't have a roadmap. The library suits my personal needs. Do you need anything that is not there? Feel free to write an issue! Those are the things I think may be interesting:
...
| options     | array / function | For the select like inputs. Value should be in options |
...
You can take a look at [defaultSkin.js](https://github.com/dgonz64/react-hook-form-auto/blob/master/src/ui/defaltSkin.js) and [`components/index.js`](https://github.com/dgonz64/react-hook-form-auto/tree/master/src/ui/components) from any skin to have a glimpse.
